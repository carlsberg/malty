...
import { ProgressSpinnerProps, ProgressSpinnerStatus } from './ProgressSpinner.types';

export const ProgressSpinner = ({ status = ProgressSpinnerStatus.Pending, dataQaId }: ProgressSpinnerProps) => {
    ...
    return (
      <svg data-testid={`${dataQaId}-failure-icon`} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          ...
          fill={theme.colors.theme.themePrimary.value}
        />
        <path
    ...
    <>
      <svg data-testid={`${dataQaId}-pending-icon`} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <g mask="url(#mask0_1664_614)">
          <rect x="24" width="24" height="24" fill={theme.colors.theme.themePrimary.value} />
        </g>
      </svg>
    </>
  );
